  ...
  for (let bi = 0; bi < bizDefs.length; bi++) {
    ...
    const { data: biz, error: bizErr } = await supabase
      ...
      .insert({
        ...
        latitude: def.template.category === "at-home-mobile" ? null : def.cityInfo.lat + (seededRandom() - 0.5) * 0.04,
        longitude: def.template.category === "at-home-mobile" ? null : def.cityInfo.lng + (seededRandom() - 0.5) * 0.04,
